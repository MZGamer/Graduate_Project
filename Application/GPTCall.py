    ...
    def messageGenerate(self, place, mode, needed, searchResult = [], startPoint = 0, perExtract = 0, restaurant : Restaurant = None, reviewList = []):
        if mode == 0:
            ...
            sendMessage =[
                            ...
                            {"role": "user", "content": f"餐廳名稱: {restaurant.name}"},
                            {"role": "user", "content": "餐廳評分:"},
                            {"role": "user", "content": f"食物份量: {restaurant.detailRating[0]}/5"},
                            {"role": "user", "content": f"服務態度: {restaurant.detailRating[1]}/5"},
                            {"role": "user", "content": f"餐廳環境: {restaurant.detailRating[2]}/5"},
